...
if (!is.null(libdir) && libdir != "None"){
  source(paste0(libdir, "/R/plotting_hg19.R"))
...
for (chr in unique(results$Chr)){
	...
	if (genomeBuild == "hg38" && file.exists(cytobandFile)){
		...
		pI <- plotIdiogram.hg38(chr, cytoband=cytoband, seqinfo=seqinfo, xlim=c(0, max(sl)), build="hg19", unit="bp", label.y=label.y, new=FALSE, ylim=ylim)
  	}
